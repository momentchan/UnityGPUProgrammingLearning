...
namespace ProceduralModeling {
    public abstract class CurveBase {
        ...
        protected float GetUtoTMapping(float u) {
            var arcLenghts = this.GetLengths();

            var il = arcLenghts.Length;

            float targetArcLenght = u * arcLenghts[il - 1];

            int i = 0, low = 0, high = il - 1;
            float comparision;

            while (low <= high) {
                i = Mathf.FloorToInt(low + (high - low) / 2f); // middle
                comparision = arcLenghts[i] - targetArcLenght;

                if(comparision < 0f) {
                    low = i + 1;
                }else if (comparision > 0f) {
                    high = i - 1;
                } else {
                    high = i;
                    break;
            ...
            if(Mathf.Approximately(arcLenghts[i], targetArcLenght)) {
                return 1f * i / (il - 1);
            }

            var lengthBefore = arcLenghts[i];
            var lengthAfter = arcLenghts[i + 1];
            var segmentLegth = lengthAfter - lengthBefore;
            var segmentFraction = (targetArcLenght - lengthBefore) / segmentLegth;
        ...
        public List<FrenetFrame> ComputeFrenetFrames(int segments, Vector3 normal, Vector3 binormal, bool closed = false) {
            ...
            if (closed) {
                theta = Mathf.Acos(Mathf.Clamp(Vector3.Dot(tangents[0], tangents[segments]), -1, 1));
                theta /= segments;
